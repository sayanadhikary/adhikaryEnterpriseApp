...
export async function storeOrder(userId, orderDetails, status){
    const saveOrder = sql`
    insert into orders (user_id, order_details, status)
    values (${userId}, ${orderDetails}, ${status})
    returning id`;
    return saveOrder;
}
...
export async function getOrdersByUserId(userId){
    const getOrders = await sql`
    select * from orders
    where user_id = ${userId}`;
    return getOrders;
}
export async function getOrderById(id){
    const getOrder = await sql`
    select * from orders
    where id = ${id}`;
    return getOrder;
}
